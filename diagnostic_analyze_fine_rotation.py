...
def main() -> int:
    ...
    rot_dir = out_dir / "rotation_refine"
    ...
    csv_path = out_dir / "fine_rotation.csv"
    ...
    lf = np.array([r["low_freq_frac"] for r in ranked], dtype=np.float64)

    fig, ax1 = plt.subplots(figsize=(9.2, 4.2), dpi=250)
    ax1.plot(thetas, uni, marker="o", label="uniformity median")
    ax1.plot(thetas, cov, marker="o", label="coverage (tile frac)")
    ax1.plot(thetas, lf, marker="o", label="low-freq power frac")
    ...
    fig.tight_layout()
    fig.savefig(out_dir / "fine_rotation_comparison.png", bbox_inches="tight")
    plt.close(fig)

    (out_dir / "BEST_ROTATION.json").write_text(json.dumps({"BEST_ROTATION": best, "all": records}, indent=2), encoding="utf-8")
    return 0
